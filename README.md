...
- Schedule, line-up and host bios are hardcoded in the `<script>` block near the bottom of `index.html` (`SCHEDULE`, `ROLE_POOL`) — update them there as the programme changes. Doors are **Sun 16 Aug 2026, 12:00 midday, running until 02:00** (i.e. into Monday); the `lineup` array on that event drives the Line-up section (sound healing 15:00, connection game 17:00, then DJs Camilo / Leo / Tachi Ale / Soe Music through to 01:00). The 31 Aug party has no `lineup` yet, and the section renders only for the first event that has one.
